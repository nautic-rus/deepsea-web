...
export class DoclistNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cols = [
      ...
      {field: 'contract', header: 'Договор', sort: true, width: '150px', visible: this.auth.getUser().groups.includes('Nautic')},
      ...
      {field: 'period', header: 'Этап', sort: true, width: '150px', visible: this.auth.getUser().groups.includes('Nautic')},
      {field: 'contract_due_date', header: 'Срок исполнения', sort: true, width: '150px', visible: this.auth.getUser().groups.includes('Nautic')},
      {
        field: 'issue_comment',
        header: 'Комментарий',
        sort: true,
  ...
  showProjectIssues(project: string) {
    this.issueManager.getDoclistByProject(project).subscribe(res => {
      ...
      if (this.auth.getUser().groups.includes('customers')) {
        // @ts-ignore
        res = res.filter(issue => {
          return issue.issue_type === 'RKD' || issue.issue_type === 'PDSP';
        })
      }
      console.log(res)
